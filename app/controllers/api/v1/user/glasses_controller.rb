class Api::V1::User::GlassesController < ApplicationController
        ...
        def create
          if @frame.stock >= 1 && @lense.stock >= 1
            @price = set_custom_glass_price
            @custom_glass = Glass.new(lense_id: params[:lense_id].to_i, frame_id: params[:frame_id].to_i, price: @price)
        ...
        def reduce_stock_count
          @frame = Frame.find_by(id: @custom_glass.frame_id).stock -1
          @lense = Lense.find_by(id: @custom_glass.lense_id).stock -1
          @frame.save
          @lense.save
        end  
